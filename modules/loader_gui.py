...
class ColumnMapperDialog(tk.Toplevel):
    def __init__(self, parent, columns, filename):
        ...
        ttk.Label(frame_x, text="X-Axis (Time):").pack(side=tk.LEFT)
        ...
        ttk.Label(frame_y, text="Y-Axis (Volts):").pack(side=tk.LEFT)
        ...
        frame_fs = ttk.LabelFrame(main_frame, text="Settings", padding=10)
        frame_fs.pack(pady=15, fill=tk.X)
        
        ttk.Label(frame_fs, text="Sampling Freq (Hz):").pack(side=tk.LEFT)
        # Set default value to 250000
        self.fs_var = tk.StringVar(value="250000")
        self.fs_entry = ttk.Entry(frame_fs, textvariable=self.fs_var, width=15)
